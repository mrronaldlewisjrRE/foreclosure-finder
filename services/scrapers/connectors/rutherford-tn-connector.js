...
class RutherfordTNConnector extends CountyConnectorBase {
  constructor() {
    super({
      countyCode: 'TN_RUTHERFORD',
      state: 'TN',
  ...
  async executeSearch(page, date) {
    console.log(`[TN_RUTHERFORD Connector] Accessing live source at ${this.sourceUrl} for ${date}...`);
    return true;
  }

  async extractRecords(countyCode, date) {
    console.log(`[TN_RUTHERFORD Connector] Querying live notices from Better Choice Notices...`);
    // State 44 = TN, County 75 = Rutherford
    const notices = [];
    let pageNumber = 1;
    let hasMore = true;
    const pageSize = 100;
    ...
    while (hasMore) {
      const listUrl = `https://api.betterchoicenotices.com/api/notices/?stateId=44&countyId=75&page=${pageNumber}&pageSize=${pageSize}`;
      console.log(`[TN_RUTHERFORD Connector] Fetching page ${pageNumber} of notices from Better Choice Notices...`);
    ...
    console.log(`[TN_RUTHERFORD Connector] Loaded ${notices.length} notices from BCN.`);
    
    if (notices.length === 0) {
      throw new Error(`No notices found for Rutherford County from live source API`);
    }
    ...
    for (const notice of notices) {
      try {
        console.log(`[TN_RUTHERFORD Connector] Processing notice ID: ${notice.id} (${notice.property_address})...`);
        ...
        if (!docResult.preSignedURL) {
          console.warn(`[TN_RUTHERFORD Connector] Failed to get S3 PDF URL for notice ID ${notice.id}`);
          continue;
        }
        ...
        if (parsed.error) {
          console.warn(`[TN_RUTHERFORD Connector] Python PDF parser failed for notice ID ${notice.id}: ${parsed.error}`);
          continue;
        }
        ...
        const record = {
          ...
          propertyAddress: {
            ...
            city: parsed.city || 'MURFREESBORO',
            state: parsed.state || 'TN',
            zip: parsed.zip || '37130'
          },
          mailingAddress: {
            street: parsed.street || notice.property_address,
            city: parsed.city || 'MURFREESBORO',
        ...
        console.error(`[TN_RUTHERFORD Connector] Error processing notice ID ${notice.id}:`, err.message);
      }
    }
    
    console.log(`[TN_RUTHERFORD Connector] Successfully parsed ${records.length} notices for ingestion.`);
    return records;
  }
}

module.exports = RutherfordTNConnector;
